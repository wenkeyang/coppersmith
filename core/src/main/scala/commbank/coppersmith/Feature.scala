...
object Feature {
  ...
  implicit object ContinuousFloatingPoint
      extends Conforms[Type.Continuous.type, Value.FloatingPoint]
  ...
  object Metadata {
    ...
    case class TypeInfo(typeName: String, typeArgs: List[TypeInfo]) {
      override def toString =
        typeName + typeArgs.toNel.map(_.list.mkString("[", ",", "]")).getOrElse("")
    }
    object TypeInfo {
      def apply[T : TypeTag]: TypeInfo = TypeInfo(implicitly[TypeTag[T]].tpe)
      def apply(t: ScalaType): TypeInfo =
        TypeInfo(t.typeSymbol.fullName, t.typeArgs.map(TypeInfo(_)))
    }
...
object FeatureValue {
  implicit class AsEavt(fv: FeatureValue[Value]) {
    def asEavt(time: FeatureTime): (EntityId, Name, Value, FeatureTime) =
      (fv.entity, fv.name, fv.value, time)
  }
}
