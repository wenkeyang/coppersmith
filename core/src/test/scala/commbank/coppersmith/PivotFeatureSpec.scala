...
"""
  ...
  def generateFeatureValues = forAll { (c: Customer) => {
    ...
    featureValues must_== List(
      ...
      FeatureValue[Decimal] (c.id, CustomerFeatureSet.height.metadata.name, c.height),
      FeatureValue[Decimal] (c.id, CustomerFeatureSet.credit.metadata.name, c.credit)
    )
  }}
}
...
"""
  ...
  def pivot(
    ...
    if (field == Fields[Customer].Name) {
      Patterns.pivot[Customer, Str, String](ns, ft, e, Fields[Customer].Name, desc)
    } else if (field == Fields[Customer].Age) {
      Patterns.pivot[Customer, Integral, Int](ns, ft, e, Fields[Customer].Age, desc)
    } else if (field == Fields[Customer].Height) {
      Patterns.pivot[Customer, Decimal, Double](ns, ft, e, Fields[Customer].Height, desc)
  ...
  def metadataNamespace = forAll {
    (namespace: Namespace, desc: Description, fType: Type, field: Field[Customer, _]) => {
      val feature = pivot(namespace, desc, fType, _.id, field)
