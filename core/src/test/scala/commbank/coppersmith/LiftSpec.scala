...
class LiftSpec extends Specification {
  ...
  def join1 = {
    ...
    val pipeWithJoin: (NextPipe[B, B], (A :: HNil => Int, B => Int)) = (NextPipe[B, B](bs), ((a: A :: HList) => a.head.id, (b: B) => b.id))
  ...
  def join2 = {
    ...
    val pipeWithJoin: (NextPipe[C,C], (A :: B :: HNil => String, C => String)) = (NextPipe[C, C](cs), ((x : A :: B :: HNil) => x.tail.head.str, (c: C) => c.str))
  ...
  def fold1 = {
    ...
    val initial: List[A :: HNil] = as.map(_ :: HNil)
    val pipeWithJoin: (NextPipe[B, B], (A :: HNil => Int, B => Int) ) = (NextPipe[B,B](bs),  ((a: A :: HNil) => a.head.id, (b: B) => b.id))
//
