...
case class FluentMovieFeaturesConfig(conf: Config) extends FeatureJobConfig[Movie] {
  ...
  val featureSource  = FluentMovieFeatures.source.bind(from(movies))
...
object FluentMovieFeaturesJob extends SimpleFeatureJob {
  def job = generate(FluentMovieFeaturesConfig(_), FluentMovieFeatures)
}
