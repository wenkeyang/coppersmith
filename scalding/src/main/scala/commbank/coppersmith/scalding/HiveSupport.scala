...
object HiveSupport {
  ...
  def writeTextTable[
    ...
    val partitioned: TypedPipe[(P, String)] =
      pipe.map(v => partition.extract(v) -> serialise[T](v, conf.delimiter, "\\N")(conf.dcs))
      ...
      .flatMap(tempDir => {
        val sink = PartitionedTextLine(tempDir.toString, partition.pattern);
        {
          for {
            ...
            oPValues <- partitioned
                         .aggregate(Aggregator.toSet.composePrepare(_._1))
                         .toOptionExecution
            oPartitions = oPValues
              .map(_.toSet.toList)
              .toList
              .flatten
              .toNel
              .map(pValues => Partitions(conf.partition, pValues.head, pValues.tail: _*))
  ...
  private def moveToTarget(
    ...
    partitionRelPaths
      .foldLeft[WriteResult](Execution.from(Right(Set())))((acc, partitionRelPath) =>
            acc.flatMap {
              ...
              case false =>
                Execution.fromHdfs(
                    for {
                      ...
                      _ <- parts
                            .map(part => Hdfs.move(part, new Path(targetPartition, part.getName)))
                            .sequence
