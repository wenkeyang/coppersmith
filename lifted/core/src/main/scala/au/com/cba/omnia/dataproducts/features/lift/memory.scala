...
trait MemoryLift extends Lift[List] with Materialise[List, ({type λ[-α] = α => Unit})#λ, ({type λ[α] = Unit => α})#λ]{
  def lift[S,V <: Value](f:Feature[S,V])(s: List[S]): List[FeatureValue] = {
    s.flatMap(s => f.generate(s))
  }

  def lift[S](fs: FeatureSet[S])(s: List[S]): List[FeatureValue] = {
  ...
  (leftSrc:List[A], rightSrc:List[B], sink: (FeatureValue) => Unit) =
    materialise[(A,B), V](feature)(liftJoin(joined)(leftSrc, rightSrc), sink)

  def materialise[S, V <: Value](f:Feature[S, V])(src:List[S], sink: FeatureValue => Unit): Unit => Unit = _ => {
    lift(f)(src).foreach(sink)
  }

  def materialise[S](featureSet: FeatureSet[S])(src:List[S], sink: FeatureValue => Unit): Unit => Unit = _ => {
