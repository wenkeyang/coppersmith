...
trait Lift[P[_]] {
  def lift[S,V <: Value](f:Feature[S,V])(s: P[S]): P[FeatureValue]

  def lift[S](fs: FeatureSet[S])(s: P[S]): P[FeatureValue]
...
trait Materialise[P[_], O[_], IO[_]] { this: Lift[P] =>
    def materialiseJoinFeature[A, B, J : Ordering, V <: Value]
        ...
        (leftSrc:P[A], rightSrc:P[B], sink: O[FeatureValue]): IO[Unit]

    def materialise[S,V <: Value](f:Feature[S,V])(src:P[S], sink: O[FeatureValue]): IO[Unit]

    def materialise[S](featureSet: FeatureSet[S])(src:P[S], sink: O[FeatureValue]): IO[Unit]
}
